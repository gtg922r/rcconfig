...
Install nvm (latest stable) with no default Node version — available as a fallback for projects needing a different version. System `/usr/bin/node` is primary; use `nvm install <ver>` only when needed.
